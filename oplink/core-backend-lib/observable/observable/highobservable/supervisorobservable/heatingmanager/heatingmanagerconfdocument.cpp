...
oplink::HeatingManagerConfDocument::HeatingManagerConfDocument(oplink::HeatingManagerLoaderHook& browserHook):
    oplink::HighObservableConfDocument{browserHook}
{
...
{
    bool ret{false};
    QString observableName,localisation,title,state,control;
    ...
    control = hmElem.attribute(controlAttr());
    if (controlType_pw() == control)
    {
        controlType = HeatingManagerLoaderHook::ControlType::PW;
    }
    else if (controlType_pwt() == control)
    {
        controlType = HeatingManagerLoaderHook::ControlType::PWT;
    }
    state = hmElem.attribute(stateAttr());
    stateOn = val_on() == state;
                                                                       ...
                                                                       oplink::HeatingManagerLoaderHook::ControlType ctrT)
{
    ...
    if (descendants.size() == 1) // only one control element expected !
    {
        ...
        setPointValue = controlElem.attribute(setPointAttr());
        ...
        ret = hook().controlDeclarationBegin(setPointValue,threshold,max_temp);
        if (ret && (oplink::HeatingManagerLoaderHook::scheduledSetPoint() == setPointValue))
        {
...
bool oplink::HeatingManagerConfDocument::extractDataFromRoomElement(QDomElement &roomElem)
{
    ...
    wod = roomElem.attribute(windowOpeningDetectionAttr()) == val_yes();
    ret = hook().roomDeclarationBegin(roomName,wod);
    if (ret)
    {
                                                  ...
                                                  const QString& observableName)
{
    ...
    if (descendants.size() == 1) // Only one optional scheduler is expected
    {
        ...
        QString buffer;
        QTextStream tstream{&buffer};
        elem.save(tstream, 1);
        plugframe::SchedulerElementHook& schedulHook{hook().schedulerDeclarationBegin(buffer)};
