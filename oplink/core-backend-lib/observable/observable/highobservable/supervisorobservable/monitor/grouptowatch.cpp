...
oplink::GroupToWatch::GroupToWatch(oplink::QspGroupToWatchAlgorithm algo):
    m_algorithm{algo}
{
...
void oplink::GroupToWatch::algorithm(GroupToWatchAlgorithm *algo)
{
    m_algorithm.reset(algo);
    m_algorithm->group(this);
}
...
void oplink::GroupToWatch::addCategory(const QString &categoryName)
{
    ...
    {
        oplink::QspStateToWatchList newCat{new oplink::StateToWatchList};
...
bool oplink::GroupToWatch::isCategoryEmpty(const QString &categoryName)
{
    ...
    if (m_categories.contains(categoryName))
    {
        oplink::QspStateToWatchList cat{m_categories.value(categoryName)};
...
void oplink::GroupToWatch::addMonitoredState(const QString &categoryName, oplink::QspStateToWatch monitoredState)
{
    if (m_categories.contains(categoryName))
    {
        oplink::QspStateToWatchList category{m_categories.value(categoryName)};
...
oplink::QspStateToWatchList oplink::GroupToWatch::category(const QString &categoryName)
{
    return m_categories.value(categoryName);
}

void oplink::GroupToWatch::setObservableService(oplink::ObservableServiceInterface *observableService)
{
    if (!m_algorithm.isNull())
    {
        m_algorithm->observableService(observableService);
    }
}

void oplink::GroupToWatch::subscribe()
{
    if (!m_algorithm.isNull())
    {
        m_algorithm->subscribe();
    }
}

void oplink::GroupToWatch::unsubscribe()
{
    if (!m_algorithm.isNull())
    {
        m_algorithm->unsubscribe();
    }
}
...
{
    oplink::QspStateToWatchList msList{category(categoryName)};
