...
message(STATUS "OL_BUILD_BACKEND = ${OL_BUILD_BACKEND}")
message(STATUS "OL_BUILD_FRONTEND = ${OL_BUILD_FRONTEND}")
...
if(NOT DEFINED OL_BUILD_BACKEND)
  message(FATAL_ERROR "OL_BUILD_BACKEND is not set. Must be set to ON or OFF")
endif()
if(NOT DEFINED OL_BUILD_FRONTEND)
  message(FATAL_ERROR "OL_BUILD_FRONTEND is not set. Must be set to ON or OFF")
endif()
...
if (OL_BUILD_FRONTEND)
  ...
  if(NOT DEFINED PF_GUI_LIB_FILE OR PF_GUI_LIB_FILE STREQUAL "")
    message(FATAL_ERROR "PF_GUI_LIB_FILE is not set")
  endif()
...
set(OL_BACKEND_DIR "${OL_BIN_DIR}/oplink_backend")
set(OL_BACKEND_BUNDLES_DIR "${OL_BACKEND_DIR}/bundles")
set(OL_BACKEND_CONF_DIR "${OL_BACKEND_DIR}/conf")
set(OL_BACKEND_LOGS_DIR "${OL_BACKEND_DIR}/logs")
# runtime guiconsole plugframe directories
set(OL_FRONTEND_DIR "${OL_BIN_DIR}/oplink_frontend")
set(OL_FRONTEND_BUNDLES_DIR "${OL_FRONTEND_DIR}/bundles")
set(OL_FRONTEND_CONF_DIR "${OL_FRONTEND_DIR}/conf")
set(OL_FRONTEND_LOGS_DIR "${OL_FRONTEND_DIR}/logs")

# remove old configuration
##########################
if(NOT OL_BUILD_BACKEND)
  file(REMOVE_RECURSE
    "${OL_LIBS_DIR}"
    "${OL_BACKEND_DIR}"
  )
...
########################
if (OL_BUILD_BACKEND OR OL_BUILD_FRONTEND)
...
if (OL_BUILD_BACKEND)
  ...
  # backend's bundles
  ###################
  ol_copy_bundle("backend"  "framework"               "${PF_PLUGIN_FRAMEWORK_FILE}")
  ol_copy_bundle("backend"  "logger"                  "${PF_PLUGIN_LOGGER_FILE}")
  ...
  ol_copy_conf_file(
    ...
    "${OL_BACKEND_BUNDLES_DIR}/logger/conf"
  )
  ...
  ol_copy_conf_file(
    ...
    "${OL_BACKEND_BUNDLES_DIR}/enoceaninfrastructure/conf"
  )
  ...
  ol_copy_conf_file(
    ...
    "${OL_BACKEND_BUNDLES_DIR}/observablemodelloader/conf"
  )

  # Tcpbackend bundle conf
  ol_copy_conf_file(
    "${OL_PROFILE_DIR}/oplink/backend/listen_addr.ini"
    "${OL_BACKEND_BUNDLES_DIR}/tcpbackend/conf"
  )
endif()
...
if (OL_BUILD_FRONTEND)
  # frontend's lib
  ################
  ol_copy_lib(${PF_GUI_LIB_FILE} ${PF_GUI_LIB_SONAME} ${PF_GUI_LIB_LINKER})

  # frontend's bundles install
  #################
  ol_copy_bundle("frontend" "framework"   "${PF_PLUGIN_FRAMEWORK_FILE}")
  ol_copy_bundle("frontend" "logger"      "${PF_PLUGIN_LOGGER_FILE}")
  ...
  ol_copy_conf_file(
    ...
    "${OL_FRONTEND_BUNDLES_DIR}/logger/conf"
  )

  # Guiconsole bundle conf
  ol_copy_conf_file(
    "${OL_PROFILE_DIR}/oplink/frontend/guiconsole.ini"
    "${OL_FRONTEND_BUNDLES_DIR}/guiconsole/conf"
  )
endif()
